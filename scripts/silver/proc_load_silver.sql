...
PURPOSE: Clean, transform and load silver <table> from bronze schema.
...
DECLARE @start_time DATETIME2, @end_time DATETIME2, @batch_start DATETIME2, @batch_end DATETIME2;
...
SELECT [prd_id]
  ...
  --NOT IN ( SELECT distinct id FROM bronze.crm_prd_info );
...
TRUNCATE TABLE silver.crm_sales_detail;
PRINT '>> Inserting Data: silver.crm_sales_detail'
...
PRINT '>> Inserting Data: silver.erp_cust_az12'
  ...
  SELECT
   ...
   ,CASE WHEN [bdate] > GETDATE() THEN NULL 
      ELSE bdate 
	  END AS bdate
   , CASE WHEN UPPER(TRIM([gen])) IN ('F', 'FEMAIL') THEN 'Female'
